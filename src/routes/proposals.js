...
// Campo personalizado "Celular" no Bitrix (onde o telefone realmente fica gravado nos cards).
const BITRIX_CAMPO_CELULAR = 'UF_CRM_1719946624905';
...
// Deixa só os dígitos e padroniza para os últimos 11 (DDD + 9 dígitos),
// removendo um eventual "55" do país. Assim "(84) 99912-8605", "84999128605"
// e "5584999128605" viram todos "84999128605" na hora de comparar.
function normalizarTelefone(valor) {
  const d = String(valor || '').replace(/\D/g, '');
  return d.length > 11 ? d.slice(-11) : d;
}

// Procura um card JÁ EXISTENTE no funil COMERCIAL (CATEGORY_ID 12) pelo telefone,
// olhando o campo personalizado "Celular" — que é onde o telefone fica de verdade.
async function buscarDealComercialPorTelefone(webhookUrl, telefoneRaw) {
  const alvo = normalizarTelefone(telefoneRaw);
  if (alvo.length < 10) return null; // telefone curto/ inválido → não arrisca

  // Usa os últimos 4 dígitos como "contém" pra reduzir a lista que volta do Bitrix
  // (esses 4 dígitos ficam grudados mesmo no número formatado, ex.: ...-8605).
  const ultimos4 = alvo.slice(-4);

  const resp = await bitrixPost(webhookUrl, 'crm.deal.list', {
    filter: {
      CATEGORY_ID: 12,
      ['%' + BITRIX_CAMPO_CELULAR]: ultimos4,
    },
    select: ['ID', BITRIX_CAMPO_CELULAR],
    order:  { ID: 'DESC' },
  });

  const deals = (resp && Array.isArray(resp.result)) ? resp.result : [];
  for (const d of deals) {
    if (normalizarTelefone(d[BITRIX_CAMPO_CELULAR]) === alvo) {
      return d.ID;
    }
  }
  return null;
...
async function criarNegocioBitrix(prop, vendedor) {
  try {
    ...
    const dealExistente = await buscarDealComercialPorTelefone(webhookUrl, prop.telefone);
    if (dealExistente) {
    ...
    const payload = {
      fields: {
        ...
        PHONE: prop.telefone
          ...
          : undefined,
        // Grava também no campo "Celular" — assim a anti-duplicidade funciona
        // também na ordem inversa (proposta primeiro, visita depois).
        [BITRIX_CAMPO_CELULAR]: prop.telefone || undefined,
      }
    };
