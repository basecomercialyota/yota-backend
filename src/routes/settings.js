...
// Chaves permitidas
const ALLOWED_KEYS = ['system_wa', 'email_whitelist', 'kits_overrides', 'kits_version', 'bitrix_webhook_url', 'bitrix_users', 'batalha_config', 'dash_sid', 'dash_key', 'dash_sheet'];
...
router.put('/:key', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    // Sempre que os PREÇOS mudam, sobe a versão. É o que faz o app do
    // consultor descartar qualquer preço antigo guardado no navegador.
    if (key === 'kits_overrides') {
      const versao = Date.now();
      await Setting.findOneAndUpdate(
        { key: 'kits_version' },
        { key: 'kits_version', value: versao, updatedAt: new Date() },
        { upsert: true, new: true }
      );
    }
