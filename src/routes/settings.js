...
// Chaves permitidas
const ALLOWED_KEYS = ['system_wa', 'email_whitelist', 'kits_overrides', 'bitrix_webhook_url', 'bitrix_users', 'batalha_config', 'dash_sid', 'dash_key', 'dash_sheet', 'funil_sdr_periodos'];
...
router.put('/:key', auth, adminOnly, async (req, res) => {
  ...
  if (key === 'funil_sdr_periodos') {
    if (!Array.isArray(value)) {
      return res.status(400).json({ erro: 'funil_sdr_periodos deve ser um array' });
    }
  }
